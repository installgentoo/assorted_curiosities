...
/// macros have a benefit of being separate color from keywords, which does
/// improve visibility for constants not usable for existing codebase, but i am
/// imagining that i'm setting codesyle here as you might notice further on i
/// do like my constants
...
bool MessageStore::SendMessage(string sender, string recipient,
                               string message) {
  ...
  sender_messages.emplace_back(Message{move(sender), move(recipient),
                                       move(message), std::time(nullptr)});
  return true;
}

bool MessageStore::RecieveMessages(
    string recipient, std::function<bool(Message const &)> reception_callback) {
  ...
  for (auto &sender : m_user_messages) {
    ...
    sender_messages.erase(std::remove_if(sender_messages.begin(),
                                         sender_messages.end(),
                                         reception_callback),
                          sender_messages.cend());
  }
...
bool message_store::ProcessMessaging(MessageStore &message_store) {
  ...
  case 4: {
    for (VAL &sender : message_store.userMessages()) {
      ...
      PRINTLN("Sender '" << sender.first << "', (" << messages.size()
                         << " messages):");
      ...
      VAL message_its = [&] {
        ...
        std::sort(its.begin(), its.end(), [](VAL &l, VAL &r) {
          return l->recipient != r->recipient ? l->recipient < r->recipient
      ...
      for (VAL &message : message_its) {
        PRINTLN(
            "    " << std::put_time(std::gmtime(&message->timestamp), "%c %Z")
