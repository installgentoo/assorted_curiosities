...
bool contains_loops(node *head) {
  ...
  node *slow = head, *fast1 = head, *fast2;

  // check if 2x width frame bumps into slower one
  while ((fast2 = fast1->next) && (fast1 = fast2->next)) {
...
// 2. worst case scenario minimum number will be N+1, therefore we can mark
// every number from 1 to N+1 in a temp array, and take the first one unmarked
...
// 3. we can walk from both ends and build up sums for each position. then we
...
int split(vector<int> v) {
  ...
  {
    int sum = 0;
    std::transform(v.begin(), v.end(), back_inserter(fwrd), [&](auto v) {
      sum += v;
      return sum;
    });
  }
  ...
  {
    int sum = 0;
    std::transform(v.rbegin(), v.rend(), back_inserter(back), [&](auto v) {
      sum += v;
      return sum;
    });
  ...
  int min_val = std::numeric_limits<int>::max(), mid;
