...
int main(int argc, const char **argv) {
  ...
  // i have a fast implementation of voronoi segmentation on gpu, but not for
  // an arbitrary field size i won't bother with yet another voronoi
  // implementation, we all know how to google existing algos instead i'll use
  // a method i just came up with, because i find it amusing i'll search
  // amongst nearest points by x and y until their x and y distances become
  // bigger than current shortest following code is extremely simple so i'm not
  // gonna comment on it
  ...
  vector<pair<SortedBy::const_iterator, SortedBy::const_iterator>>
      sorted_points;
  ...
  const auto async_find_nearest = [thread_num, &sorted_points, &pointPairs,
                                   &by_x, &by_y](uint thread_id) {
    for (size_t i = thread_id; i < sorted_points.size(); i += thread_num) {
      const auto p = sorted_points[i];
      const auto point = *p.first;

      auto x_l = p.first, x_r = x_l, y_l = p.second, y_r = y_l;

      bool x_l_is = true, x_r_is = true, y_l_is = true, y_r_is = true;

      const auto dst_x = [](pointIter l, pointIter r) {
        return std::abs(l->m_x - r->m_x);
      };
      const auto dst_y = [](pointIter l, pointIter r) {
        return std::abs(l->m_y - r->m_y);
      };
      const auto dst = [](pointIter l, pointIter r) {
        double x = l->m_x - r->m_x, y = l->m_y - r->m_y;
      ...
      for (; x_l_is || x_r_is || y_l_is || y_r_is;) {
        x_l_is = (x_l_is && x_l != by_x.cbegin() &&
                  (dst_x(*(--x_l), point) < shortest))
                     ? true
                     : false;
        x_r_is =
            (x_r_is && ++x_r != by_x.cend() && (dst_x(*x_r, point) < shortest))
                ? true
                : false;
        y_l_is = (y_l_is && y_l != by_y.cbegin() &&
                  (dst_y(*(--y_l), point) < shortest))
                     ? true
                     : false;
        y_r_is =
            (y_r_is && ++y_r != by_y.cend() && (dst_y(*y_r, point) < shortest))
                ? true
                : false;
  ...
  const std::string result =
      PointValidation::validatePoints(solutionFilename, pointPairs) ? "Correct"
                                                                    : "Wrong";
  std::cout << std::endl;
