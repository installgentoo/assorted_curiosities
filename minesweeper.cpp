...
// i won't split program into separate files since i've been told your mail
// filters out .zip
...
#define CFAIL(text)                                                            \
  {                                                                            \
    nullopt, [&] {                                                             \
      std::ostringstream _err_s;                                               \
      _err_s << "E " << text << " |in " << __FILE__ << ":" << __LINE__         \
             << "\n";                                                          \
      return _err_s.str();                                                     \
    }()                                                                        \
  }
...
template <class To, class T> To cast(T n) {
  assert((static_cast<long double>(n) >=
...
public:
  ...
  //
  // Val is my macro for "auto const&". Why not just write it out? Macros can
  // be made more visible in your ide; also less typing. Thus we immediately
  // notice that Val declares immutable, this is a blessing for readibility.
  // Isn't this dangerous? No, clang 7+ asan will catch all dangling
  // references.
  //
  // CFAIL CRESULT result<> are basically rust's Result. Why not just use
  // exceptions? Result's more explicit, it has to be unwraped with structured
  // bindings or CTRY.

  // part 1

  static result<LevelMap> GenerateMineField(uint width, uint height,
                                            uint count) {
    ...
    Val c = [&] {
      ...
      for (uint i = 0; i < count; ++i) {
        ...
        std::swap(*std::next(candidates.begin(), i),
                  *std::next(candidates.begin(), guess));
      }
  ...
  Cell::type Open(uint x, uint y) {
    //???
    // what is this method? why does it return cell? shoudln't we return list
    // of opened cells?
  ...
  // i won't put anything else here as i've been given just these two methods
  // and asked to implement solver, not the interface
...
static string PrintMap(uint width, uint height,
                       vector<Cell::type> const &cells) {
...
// recursive Open
static void trace_open(int width, int height, int i, int j, Minefield &field,
                       vector<Cell::type> &output) {
...
static int take_a_guess(vector<Cell::type> const &cells, int width, int height,
                        uint total_mines_ingame) {
  ...
  Val yolo_ratio =
      isolated.empty() ? 9001. : double(total_mines_ingame) / isolated.size();
  Val yolo_guess = [&] {
  ...
  for (Val i : dangerous) {
    if (Val[idx, danger] = check_danger_level(i); danger < yolo_ratio)
      return idx;
  }
...
static void SolveMinefield(Minefield &field, uint total_mines_ingame) {
  ...
  // this should be returned from Open implementation, alas i've no idea what
  // that implementation is i'll pretend that we called Open and then went over
  // every cell with GetCellAt
  vector<Cell::type> cells;
  ...
  while (true) {
    Val _guess = take_a_guess(cells, cast<int>(width), cast<int>(height),
                              total_mines_ingame);
    if (_guess == -1) {
    ...
    trace_open(cast<int>(width), cast<int>(height), cast<int>(i), cast<int>(j),
               field, cells);
